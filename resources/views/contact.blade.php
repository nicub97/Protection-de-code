            ...
            <div class="glass-card rounded-3xl p-8 space-y-6">
                ...
                <form method="POST" action="{{ route('contact.store') }}" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="name" class="block text-sm font-medium text-custom-purple-light mb-2">Nom complet</label>
                        ...
                        <div>
                            <label for="email" class="block text-sm font-medium text-custom-purple-light mb-2">Email</label>
                            <input type="email" id="email" name="email" value="{{ old('email') }}" class="w-full px-4 py-3 bg-white/10 border border-custom-purple-light/30 rounded-xl text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-custom-purple-light focus:border-transparent transition-all duration-300" placeholder="Ex : prenom.nom@email.com" required>
                            @error('email')
                                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="category" class="block text-sm font-medium text-custom-purple-light mb-2">Destinataire</label>
                            <select id="category" name="category" class="w-full px-4 py-3 bg-white/10 border border-custom-purple-light/30 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-custom-purple-light focus:border-transparent transition-all duration-300" required>
                                <option value="" disabled {{ old('category') ? '' : 'selected' }}>Choisissez une option</option>
                                <option value="personnel" {{ old('category') === 'personnel' ? 'selected' : '' }}>Personnel administratif</option>
                                <option value="webmaster" {{ old('category') === 'webmaster' ? 'selected' : '' }}>Webmaster (bug)</option>
                            </select>
                            @error('category')
                                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label for="file" class="block text-sm font-medium text-custom-purple-light mb-2">Fichier joint (optionnel)</label>
                            <input type="file" id="file" name="file" disabled class="w-full px-4 py-3 bg-white/10 border border-custom-purple-light/30 rounded-xl text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-custom-purple-light focus:border-transparent transition-all duration-300 opacity-70 cursor-not-allowed">
                            <p class="text-xs text-white/60 mt-1">(Prévu pour une future évolution)</p>
                        </div>
                    </div>
                    <div>
                        <label for="message" class="block text-sm font-medium text-custom-purple-light mb-2">Message</label>
                        <textarea id="message" name="message" rows="5" class="w-full px-4 py-3 bg-white/10 border border-custom-purple-light/30 rounded-xl text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-custom-purple-light focus:border-transparent transition-all duration-300" placeholder="Décrivez votre demande ou le bug rencontré" required>{{ old('message') }}</textarea>
                        @error('message')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
